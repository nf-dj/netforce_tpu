...
class NF_SoC(SoCCore):
    def __init__(self, platform, **kwargs):
        ...
        self.submodules.uart_to_tpu = stream.Converter(8, self.nf_tpu.sink.data.nbits)
        self.comb += [
            self.uart.source.connect(self.uart_to_tpu.sink),
            self.uart_to_tpu.source.connect(self.nf_tpu.sink)
        ]

        self.submodules.tpu_to_uart = stream.Converter(self.nf_tpu.source.data.nbits, 8)
        self.comb += [
            self.nf_tpu.source.connect(self.tpu_to_uart.sink),
            self.tpu_to_uart.source.connect(self.uart.sink)
        ]
